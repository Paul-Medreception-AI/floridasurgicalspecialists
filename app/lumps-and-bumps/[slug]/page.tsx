...
import { getLumpsAndBumpsPageBySlug, lumpsAndBumpsPages, lumpsAndBumpsSlugs } from "../_data/pages";

type Params = { slug: string };

export async function generateStaticParams(): Promise<Params[]> {
  return lumpsAndBumpsSlugs.map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const page = getLumpsAndBumpsPageBySlug(slug);
...
export default async function LumpsAndBumpsDetailPage({
  params,
}: {
  params: Promise<Params>;
}) {
  ...
  const related = page.relatedSlugs
    .map((relatedSlug) => getLumpsAndBumpsPageBySlug(relatedSlug))
    .filter(Boolean);

  const moreToExplore = lumpsAndBumpsPages
        ...
        <div className="section-shell space-y-6 py-10">
          <nav className="text-sm text-slate-600">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Link href="/lumps-and-bumps" className="hover:text-slate-900">
                Lumps &amp; Bumps
              </Link>
              <span className="text-slate-400">/</span>
          ...
          <div className="space-y-10">
            <section className="space-y-3">
              ...
              <div className="space-y-4 text-slate-700 leading-relaxed">
                <p>
                  If you’re noticing changes like growth, firmness, fixation, drainage, or persistent pain, it’s
                  reasonable to schedule an evaluation.
                </p>
                <p>
                  For patients in Bradenton, Lakewood Ranch, and Sarasota, we offer office-based and outpatient options
                  depending on what’s safest for your specific lump.
                </p>
              </div>
            </section>
            ...
            <div className="mt-2 rounded-3xl border border-slate-200 bg-white p-8 shadow-md">
              ...
              <p className="mt-4 leading-relaxed text-slate-600">
                Get a clear diagnosis and a plan you can trust—often with same-week removal options when appropriate.
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link className="btn btn-outline" href="/lumps-and-bumps">
                  Back to Lumps &amp; Bumps
                </Link>
              </div>
            </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <ul className="mt-4 space-y-2 text-sm">
                {related.length > 0 ? (
                  related.map((relatedPage) => (
                    <li key={relatedPage!.slug}>
                      <Link
                        ...
                        href={`/lumps-and-bumps/${relatedPage!.slug}`}
                      >
                        {relatedPage!.h1}
                      </Link>
                    </li>
                  ...
                  <li className="text-slate-600">Explore more lump and bump topics below.</li>
                )}
              </ul>
            </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <ul className="mt-4 space-y-2 text-sm">
                {moreToExplore.map((p) => (
                  <li key={p.slug}>
                    <Link className="text-slate-700 hover:text-slate-900" href={`/lumps-and-bumps/${p.slug}`}>
                      {p.h1}
                    </Link>
                  </li>
                ))}
